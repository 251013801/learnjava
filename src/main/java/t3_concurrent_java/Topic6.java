...
public class Topic6 {
    /*
        ...
        1.synchronized
            ...
            背景：
                1."线程的阻塞和唤醒"需要CPU从用户态转为核心态，频繁的阻塞和唤醒对CPU来说是一件负担很重的工作，给系统的并发性能带来很大的压力。
            ...
            偏向锁：
                ...
                偏向锁的获取是通过CAS操作，修改 MarkWord 中的"锁状态标志位"来实现的，修改成功则获取锁，
          ...
          ————————————————————————————————————————————————————————————————————————————————————————————————————————————
            ...
            补充：
                CAS的三大问题：（1）ABA文推，（2）循环时间过长，浪费CPU，（3）只能保证一个变量的原子操作
        ...
        3.volatile
            Java内存模型（JMM，Java Memory Model）简介：
